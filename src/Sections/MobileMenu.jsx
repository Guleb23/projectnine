        ...
        >
            ...
            <div className="flex flex-col justify-between flex-1 z-10 h-full">
                ...
                <div className="flex flex-col items-start justify-center gap-6 text-white text-lg px-5 flex-1">
                    <MenuItem name="ABOUT" link="#sec" active={activeSection === '#sec'} onClick={handleClick} />
                    <MenuItem name="TECHNOLOGY" link="#four" active={activeSection === '#four'} onClick={handleClick} />
                    <MenuItem name="OUR TEAM" link="#house" active={activeSection === '#house'} onClick={handleClick} />
                    <MenuItem name="ROADMAP" link="#diagramm" active={activeSection === '#diagramm'} onClick={handleClick} />
                </div>

                {/* Кнопка снизу */}
                <div className="p-5 w-full">
                    <button className="relative overflow-hidden w-full min-h-[50px] rounded-lg text-sm md:text-[18px] font-bold text-white group flex items-center justify-center">
...
const MenuItem = ({ name, link, onClick, active }) => {
    return (
        <span
            ...
            className={`cursor-pointer transition-all duration-300 ${active ? 'border-b-2 border-white' : 'opacity-80 hover:opacity-100'}`}
        >
            {name}
        </span>
    );
